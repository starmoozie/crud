...
trait ColumnsProtectedMethods
{
    ...
    protected function hasDatabaseColumn($table, $name)
    {
        ...
        if (isset($cache[$table])) {
            ...
            $cache_name = $table.'_fillable';

            if (\Cache::has($cache_name)) {
                $column_listing = \Cache::get($cache_name);
            }
            else {
                $column_listing = \Cache::rememberForever($cache_name, function() use ($table) {
                    return $this->getSchema()->getColumnListing($table);
                });
            }

            $columns = $cache[$table] = $column_listing;
        }
